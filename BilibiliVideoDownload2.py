...
if not os.path.exists(os.path.join(WorkPath, "CookieConfig.txt")):
    ...
    with open(os.path.join(WorkPath, "CookieConfig.txt"),"r",encoding="UTF-8") as cf:
        Cookie = cf.read()

if not os.path.exists(os.path.join(WorkPath, "UserAgentHeadConfig.txt")):
    with open(os.path.join(WorkPath, "UserAgentHeadConfig.txt"),"w",encoding="UTF-8"):pass
else:
...
def ParseURL(URL: str):
    ...
    requestHead = {"User-Agent": UserAgentHead}
    ...
    with zipfile.ZipFile(os.path.join(WorkPath, ("FetchedData/" + name + ".zip")), "w") as out:
        ...
        out.writestr("视频描述.txt", describe)
